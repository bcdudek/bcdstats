...
drawplot4 <- function(rho=.7, n=20){
  ...
  plotopts2 <- theme(
    ...
    axis.title=element_text(size=16),
    title=element_text(size=16),
    panel.border=element_rect(linetype=1,size=.4,fill=NA),
    panel.grid.major=element_line(linetype=1,size=.5,color="grey95"),
  ...
  p <- ggplot(data4, aes(x=x, y=y)) + 
    ...
    labs(x="X",y="Y") 
  p <- p + plotopts2 +  
    geom_point(size=4,colour="grey95") +
    geom_point(shape=1, size=5,stroke=.2)    # Use hollow circles
...
## NEED THREE HISTORGRAMS, ONE FOR RHO=1, ONE FOR RHO=-1 AND ONE FOR THE REST
    ...
    p <- ggplot(data=data6a, aes(cp, fill=sign)) + 
      ...
      geom_histogram(color="black", alpha=.5) + theme(legend.position="none")
    p}
    else if(input$rho6 == 1){
      p <- ggplot(data=data6a, aes(cp)) + 
        plotopts2 + 
        labs(x="Cross Product Value", y="Count", title="Frequency Histogram of Cross Product Values") +
        geom_histogram(color="black", fill="#00BFC4", alpha=.5) + theme(legend.position="none")
      p}
    else if(input$rho6 == -1){
      p <- ggplot(data=data6a, aes(cp)) + 
        plotopts2 + 
        labs(x="Cross Product Value", y="Count", title="Frequency Histogram of Cross Product Values") +
        geom_histogram(color="black", fill="red", alpha=.3) + theme(legend.position="none")
      p}
}
  else if(input$plottype6 == "packed"){
    ## NEED THREE TREEMAPS, ONE FOR RHO=1, ONE FOR RHO=-1 AND ONE FOR THE REST
    if(input$rho6 < 1 & input$rho6 > -1){
    packedrectangle <- treemap(data6a, index="id1",vSize="cp2", vColor="sign",palette=c("#F8766D","#00BFC4"), type="categorical",title="Cross Product Rectangles")
    lapply(tail(grid.ls(print=FALSE)$name, 1), grid.remove)
    }
    else if(input$rho6 == 1){
      packedrectangle <- treemap(data6a, index="id1",vSize="cp2", vColor="sign", palette=c("#00BFC4"),type="categorical",title="Cross Product Rectangles")
      lapply(tail(grid.ls(print=FALSE)$name, 1), grid.remove)
    }
    if(input$rho6 == -1){
      packedrectangle <- treemap(data6a, index="id1",vSize="cp2", vColor="sign", palette=c("#F8766D"),type="categorical",title="Cross Product Rectangles")
      lapply(tail(grid.ls(print=FALSE)$name, 1), grid.remove)
    }
  }
