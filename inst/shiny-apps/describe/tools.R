...
# first, the function that permits the use of markdown in the "about" tab. NO LONGER NEEDED
...
# now some graphing and perhaps other useful functions

# dhist is a function from Denby & Mallows with slight changes in
# prsentation aspects
# when dhist is run with a=1000 it gives an equal area histogram
# when dhist is run with a at its default, it gives Denby & Mallow's 
                ...
                eps=.15, xlab = "x", plot = T,lab.spikes=T)
{
  ...
  if(plot) {
    barplot(heights, abs(diff(xbr)), space = 0, density = -1, xlab = 
              xlab, plot = TRUE, xaxt = "n",
            col="cornsilk",ylab="Frequency Counts", axes=T)
    at <- pretty(xbr)
