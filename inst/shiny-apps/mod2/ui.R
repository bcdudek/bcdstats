...
shinyUI(fluidPage(
  ...
  titlePanel(HTML("Linear Models with Two IVs:
...
All the examples presume a model with two independent variables in order to facilitate the development of a central concept in interaction analysis.
  That concept is that regardless of whether the independent variables are both categorical, one categorical and one numeric, or both numeric, the meaning of 
interaction is the same thing.  A 2-way Interaction is always the change of a bivariate relationship across values of a third variable.  In traditional
ANOVA (analyzing means with two categorical IVs), this is often termed a 'difference in differences'.<br><br>

Another central theme is the equivalence of the simple main effect concept in traditional ANOVA
 (two categorical IVs) to the simple slopes concept in moderator analysis with one or two numeric IVs.<br><br>
...
In addition, the app permits use of the mouse to rotate the 3D scatterplot figures interactively.  This provides an
...
This app is intended to be a supplement to a monograph on Interaction Analysis by B. Dudek, but can also suffice for standalone purposes.
        ...
        tabPanel(
          ...
          conditionalPanel(
            ...
            HTML("Typically Simple Main Effects would be examined only in models where an interaction is present.
                   Their depiction here is only for the sake of illustration.  The lack of an interaction is seen 
                 in the fact that the two simple main effects are the same size.")
            ),
          conditionalPanel(
            condition = (
              "input.ivtypes == 'twocat' &
            ...
            HTML("Typically Simple Main Effects would be examined only in models where an interaction is present.
                   Their depiction here is only for the sake of illustration.  The lack of an interaction is seen in the fact that 
                 the two simple main effects are the same size.")
            ),
          ########################
         ...
         HTML("The two simple main effects differ, therefore: interaction")
            ),
          conditionalPanel(
            condition = (
              "input.ivtypes == 'twocat' &
          ...
          conditionalPanel(
            ...
            HTML("The two simple main effects differ, therefore: interaction.  
                 The importance of this figure is its depiction of the simple main effects on the regression/interaction surface." )
            ),
          #############################
          ...
          conditionalPanel(
            ...
            tags$img(src="zerocat_pequod1.png", width=500)
              ),  
